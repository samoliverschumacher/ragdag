...
def create_collection(data: list[dict], collection_name: str) -> None:
    ...
    for item in data:
        points.append(
            PointStruct(
                id=item["id"], vector=item["vector"], payload={k: v for k, v in item["payload"].items() if k != "text"}
            )
        )
...
def embed_create_collection(
    client: QdrantClient, data: list[dict], collection_name: str, embed_size: int = None, encoder=None
) -> None:
    """
