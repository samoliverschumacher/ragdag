...
class AccountNumberRedactor(Process):
    ...
    @staticmethod
    def pid_scrub(request: GenerateRequest, config: dict, event_id: str) -> CleansedRequest:
        """
        ...
        try:
            ...
            cleaned_text = _remove_bank_account_number(request.user_query, replacement=config["Financial information"])
        ...
        t1 = time.time()
        logger.eval(event_id, {"metric": "clean_sensitive_info_seconds", "value": t1 - t0})
        logger.eval(event_id, {"metric": "cleaned_request", "value": cleaned_request.user_query})
    ...
    def _process(self, text: Any, data: dict, errors: ErrorStack, *_) -> tuple[Any, dict, ErrorStack, RAGStage]:
        ...
        try:
            rewritten_query = self.pid_scrub(request, self.config["redaction_map"], event_id)
            next_text: CleansedRequest = rewritten_query
        except Exception as e:
            # Give the user the retrieved references verbatim.
            errors.append((self.stage, e))
            logger.error(str(e) + "Could not remove sensitive information. Aborting.")
            raise e
