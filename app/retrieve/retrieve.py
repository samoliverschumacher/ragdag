...
class QDRANTRetriever(Process):
    ...
    def simple_retrieve(self, request: GenerateRequest, event_id: str) -> list[Context]:
        ...
        t0 = time.time()
        embedding = run_until_timeout(encode_text,
                                      self.config['embedding_timeout'],
                                      QueryEmbeddingTimeoutError, self.encoder,
                                      request.user_query)
        t1 = time.time()
        ...
        logger.eval(
            event_id, {
                "metric": "user_query_embedding",
                "value": ','.join(map(str, embedding))
            })

        if self.config['filter_on_user_metadata']:
            if hasattr(request, 'metadata') and request.metadata:
                self.config['search_filters'] = self._create_must_filter(
                    request.metadata)

        t0 = time.time()
        documents, scores, doc_ids, metadatas = self.retrieve_from_vector(
            embedding, n=self.config['top_k'])
        t1 = time.time()
        ...
        logger.eval(
            event_id, {
                ...
                "value":
                ','.join(':'.join(str(x) for x in tup)
                         for tup in zip(doc_ids, scores, strict=False))
            })
        if not documents:
        ...
        for doc, id, meta in zip(documents, doc_ids, metadatas, strict=False):
            if len(meta) > 0:
                contexts.append(
                    ContextWithMetadata(doc_id=str(id),
                                        text=doc,
                                        url=meta.get('url', ''),
                                        title=meta['title']))
            else:
    ...
    def retrieve_from_vector(
            ...
            n: int) -> tuple[list[str], list[float], list[int], list[dict]]:
        """
        ...
        metadata = [{
            ...
            if k not in {'scores', 'doc_id', 'text'}
    ...
    def _process(self, text: Any, data: dict, errors: ErrorStack,
        ...
        try:
            ...
            errors.append((self.stage, e))
            logger.error(
                str(e) + 'Applied a no-VectorStore fallback document.')
    ...
    def _fallback_context(self, msg: str='') -> list[Context]:
        return [
            Context(
                doc_id='',
                text=f'{msg}The website'
                'is an excellent resource for many related questions.')
        ]
